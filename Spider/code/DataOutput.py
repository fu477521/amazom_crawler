...
asin_type=%(asin_type)s, cart_price=%(cart_price)s, asin_state=%(asin_state)s,\
quantity=%(quantity)s, release_date=%(release_date)s, seller_id=%(seller_id)s, is_sync=0, crawler_state=1, category=%(category)s\
where asin=%(asin)s;"
...
collect_tm, getinfo_tm, image_more, variant, asin_type, cart_price, asin_state, quantity,\
release_date, seller_id, crawler_state, category) VALUES (%(asin)s,%(pasin)s,%(title)s,%(image)s,%(brand)s,%(price)s,%(sale_price)s,%(sname)s,\
%(ts_min_price)s,%(to_sell)s,%(byb)s,%(bsr)s,%(rc)s,%(rrg)s,%(r5p)s,%(r4p)s,%(r3p)s,%(r2p)s,%(r1p)s,%(feature)s,\
%(refresh_tm)s, %(collect_tm)s, %(getinfo_tm)s, %(image_more)s, %(variant)s, %(asin_type)s, \
%(cart_price)s, %(asin_state)s, %(quantity)s, %(release_date)s, %(seller_id)s, 1, %(category)s);"
...
r2p=%(r2p)s, r1p=%(r1p)s, bsr=%(bsr)s, bs1=%(bs1)s, qc=%(qc)s, qty=%(qty)s, qtydt=%(qtydt)s, tpre=%(tpre)s, is_limit=%(is_limit)s\
...
ts, byb, r5p, r4p, r3p, r2p, r1p, bsr, aday, bs1, qc, qty, qtydt, tpre, is_limit) VALUES (%(tm)s, %(asin)s, %(pasin)s, %(sn)s,\
%(pre)s, %(dpre)s, %(rc)s, %(rrg)s, %(ts)s, %(byb)s, %(r5p)s, %(r4p)s, %(r3p)s, %(r2p)s, %(r1p)s, %(bsr)s, %(aday)s,\
%(bs1)s, %(qc)s, %(qty)s, %(qtydt)s, %(tpre)s, %(is_limit)s);"
...
demo=%(demo)s, positive=%(positive)s, total_ratings=%(total_ratings)s, tm=%(tm)s,\
reivew_count=%(reivew_count)s, delivery=%(delivery)s, offering_id=%(offering_id)s,sn=%(sn)s,rank=%(rank)s,srank=%(srank)s,qty=%(qty)s, qtydt=%(qtydt)s, is_limit=%(is_limit)s, crawler_state=%(crawler_state)s where seller_id=%(seller_id)s and condition=%(condition)s \
and sname=%(sname)s and aday=%(aday)s and asin=%(asin)s and fba=%(fba)s;"
    druid_tosell_insert_sql = "INSERT INTO public.amazon_product_tosell(asin, condition, sname, stype, price,\
demo, positive, total_ratings, tm, fba, seller_id, reivew_count, delivery, aday, offering_id, sn, rank, srank, qty, qtydt, is_limit, crawler_state) VALUES (%(asin)s, %(condition)s,\
%(sname)s, %(stype)s, %(price)s, %(demo)s, %(positive)s, %(total_ratings)s, %(tm)s, %(fba)s, %(seller_id)s,\
%(reivew_count)s, %(delivery)s, %(aday)s,%(offering_id)s, %(sn)s,%(rank)s, %(srank)s,%(qty)s, %(qtydt)s, %(is_limit)s, %(crawler_state)s);"
...
rc=%(rc)s, special=%(special)s, tm=%(tm)s, issp=%(issp)s, is_prime=%(is_prime)s, fba=%(fba)s, category=%(category)s, bsr=%(bsr)s where kw=%(kw)s and aday=%(aday)s and pr=%(pr)s;"
    druid_keyword_insert_sql = "INSERT INTO public.amazon_druid_keyword_data(kw, cid, pr, asin, title, img, brand, \
msn, srn, price, rrg, rc, special, tm, aday, issp, is_prime, fba, category, bsr) VALUES (%(kw)s, %(cid)s, %(pr)s, %(asin)s, %(title)s, %(img)s, \
%(brand)s, %(msn)s, %(srn)s, %(price)s, %(rrg)s, %(rc)s, %(special)s, %(tm)s, %(aday)s, %(issp)s, %(is_prime)s, %(fba)s, %(category)s, %(bsr)s);"
...
sname='', seller_id='', is_sync=0, crawler_state=1, getinfo_tm=%(getinfo_tm)s, asin_state=0 where asin=%(asin)s;"
            print(sql)
            cur.execute(sql, data_dict)
            row = cur.rowcount
            print(1, row)
            if row > 0:
                ...
                conn.rollback()
                sql = "INSERT INTO public.amazon_product_data_tosell(asin, getinfo_tm, crawler_state, asin_state) VALUES (%(asin)s, %(getinfo_tm)s, 1, 0);"
                cur.execute(sql, data_dict)
                row = cur.rowcount
                print(2, row)
                if row > 0:
